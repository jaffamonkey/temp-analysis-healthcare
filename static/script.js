...
const STATIC_XLSX_PATH = JOB_BASE_PATH ? `${JOB_BASE_PATH}/workbook` : "accessibility_analysis.xlsx";
...
function updateCards(data) {
    document.getElementById("violations").innerText = data.violations || 0;
    document.getElementById("pages").innerText = data.pages || 0;
    document.getElementById("wcag_criteria_affected").innerText = data.distinct_wcag_criteria || 0;
    document.getElementById("impact").innerText = (data.design_system_impact || 0) + "%";
    document.getElementById("adi").innerText = data.accessibility_debt_index || 0;
    document.getElementById("opportunity").innerText = (data.accessibility_opportunity_score || 0) + "%";
    document.getElementById("pageChart").innerText = data.issuesperpage || 0;
    document.getElementById("frame_issues").innerText = (data.frame_issues || 0) + " frames";
    document.getElementById("frame_pages").innerText = (data.frame_pages || 0) + " affected pages";
    document.getElementById("frame_pages_list").innerText = data.frame_pages_list || 0;
    document.getElementById("problemTypeChart").innerText = data.problem_types || 0;
    document.getElementById("shared_source_rate").innerText = (data.shared_source_rate || 0) + "%";
    document.getElementById("top5_page_concentration").innerText = (data.top5_page_concentration || 0) + "%";
    document.getElementById("top5_pages_list").innerText =
        (data.top5_pages_list && data.top5_pages_list.length)
            ? data.top5_pages_list.join(", ")
            : "-";

    const conf = data.confidence_counts || {};
    document.getElementById("conf_high").innerText = conf.high || 0;
    document.getElementById("conf_medium").innerText = conf.medium || 0;
    document.getElementById("conf_low").innerText = conf.low || 0;

    const cons = data.consensus_counts || {};
    document.getElementById("cons_verified").innerText = cons.verified || 0;
    document.getElementById("cons_likely").innerText = cons.likely || 0;
    document.getElementById("cons_single").innerText = cons.single || 0;
}
...
function renderProblemTypeChart(data) {
    ...
    problemTypeChart = buildChart(
        ...
        (label) => {
            const map = {
                "Forms": ["form", "search", "form_field", "file_upload"],
                "Interactive": ["button", "pseudo_button", "modal", "dialog", "accordion", "carousel", "tooltip", "theme_toggle"],
                "Navigation": ["navigation", "tabs", "breadcrumb", "pagination", "link", "navbar", "dropdown_menu"],
                "Content": ["heading", "text", "list", "card", "product_card", "alert", "selectable_list"],
                "Media": ["image", "icon"],
                "Structure": ["layout", "grid", "frame", "iframe_embed", "table", "data_table", "document_metadata"],
                "ARIA": ["aria"]
            };

            const knownComponents = new Set(Object.values(map).flat());

            const matches = GLOBAL_ROWS.filter(r => {
                const comp = (r.component || "other").toLowerCase();

                if (label === "Other") {
                    return !knownComponents.has(comp);
                }
...
function renderPageChart(data) {
    ...
    pageChart = buildChart(
        ...
        (label) => {
            const matches = GLOBAL_ROWS.filter(
                r => (r.page || "unknown") === label
            );

            const hasFrames = matches.some(r => r.component === "frame");

            const title = hasFrames
                ? `Page: ${label} ⚠️ (Frames detected)`
                : `Page: ${label}`;

            showDrilldown(title, matches);
        }
    );
}
...
function renderWCAGRuleChart(clusters) {
    ...
    wcagRuleChart = buildChart(
        ...
        "WCAG Rule Breakdown",
        (label) => {
            const matches = GLOBAL_ROWS
                .filter(r => normalizeWcagCode(r.wcag) === label)
                .map(r => ({
                    ...r,
                    page: r.page || r.page_key || "",
                    page_display: r.page_display || r.page || r.page_key || "Unknown page"
                }));

            showDrilldown("WCAG: " + label, matches);
        }
    );
}
...
function renderDrilldownPage() {
    ...
    pageItems.forEach(i => {
        const ruleLabel = escapeHtml(i.rule_label || i.rule_name || i.ruleId || i.rule_id || i.wcag_title || i.wcag || "Unknown");
        const componentLabel = escapeHtml(i.component_display || i.component || "Other");
        const issueScope = escapeHtml(i.issue_scope || "Unknown");
        const patternLabel = escapeHtml(i.display_pattern || i.pattern || "");
        const pageLabel = escapeHtml(i.page_display || i.page || i.page_name || "Unknown page");
        const sourceLabel = escapeHtml(i.source || (Array.isArray(i.sources) ? i.sources.join(", ") : i.sources || "-"));
        const messageLabel = escapeHtml(i.message || i.description || "");
        const domPathLabel = escapeHtml(i.dom_path || i.selector || i.xpath || i.path || i.dom || "");
...
function showNextBestFixDrilldown(patternEncoded, labelEncoded) {
    ...
    const label = decodeURIComponent(labelEncoded || "") || pattern || "Unknown pattern";
    const matches = GLOBAL_ROWS.filter(r => (r.pattern || "") === pattern);
    const title = `Fix Candidate: ${label}`;
    showDrilldown(title, matches);
}
